...
McClintock provides scripts to automatically simulate genomes with a single synthetic TE insertions, generate synthetic whole genome sequence (WGS) paired-end datasets, analyze synthetic WGS datasets using the McClintock meta-pipeline, and summarize component method performance.
...
## Quick Start
- A sample script to submit the entire simulation and evaluation framework to a SLURM cluster job can be found as follows. The basic process is (i)) generate a configuration file with settings for the simulation using `make_snakemake_config.py`, (ii) run a primary Snakemake workflow that submits jobs to a HPC cluster that simulate genomes, generate synthetic paired-end datasets, and runs McClintock on synthetic datasets, then (iii) run a secondary Snakemake workflow that summarizes the results of the simulations.
- Important default settings for the example analysis below are generated by `make_snakemake_config.py` are as follows. Other default settings (e.g. TSD lengths, simulated read length, simulated error rate) are described in detail in Section ["Creating configuration for simulation"](https://github.com/bergmanlab/mcclintock/edit/documentation/simulation/README.md#1-creating-configuration-for-simulation). 
  - Depth of simulated reads: 3x, 6x, 12x, 25x, 50x, 100x.
  - Number of replicates per depth: 30.
  - Strand of sythetic insertion: forward and reverse (i.e. two synthetic genomes are created for each candidate insertion site: one in the forward orientation, one in the reverse orientation).
