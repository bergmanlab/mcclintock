...
def main():
    ...
    for x in range(args.start,args.end+1):
        ...
        if not os.path.exists(fastq1) or not os.path.exists(fastq2):
            ...
            fastq1, fastq2 = create_synthetic_reads(modified_reference, args.coverage, num_pairs, args.length, args.insert, args.error, x, args.out, run_id=args.runid, seed=args.seed)
...
def create_synthetic_reads(reference, coverage, num_pairs, length, insert, error, rep, out, run_id="", seed=None):
    if seed is not None:
